 ...
 * reference: RFC 1321
 */
joyee::MD5& joyee::MD5::update(uint8_t const* input, uint32_t inputLen) {
    // compute number of bytes mod 64
    uint32_t index = (uint32_t) ((lo >> 3) & 0x3F);
 ...
 */
joyee::MD5& joyee::MD5::update(char const* input, uint32_t inputLen) {
    return update((uint8_t const*) input, inputLen);
}
 ...
 */
std::string joyee::MD5::toString() const {
    if (!finalized)
        return std::string("");
 ...
 * reference: RFC 1321 3.4
 */
void joyee::MD5::transform(uint8_t const block[BLOCK_SIZE]) {
...
std::string joyee::md5(std::string const str) {
    MD5 md5 = MD5();
